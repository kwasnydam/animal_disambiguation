...
class TestProcessText(unittest.TestCase):
    def setUp(self):
        self.tested_function = tested_module.process_text

    def test_outputs_list_of_sentences_given_string_containing_word_mouse(self):
        test_sentence = 'mouse is an animal.'
        self.assertTrue(type(self.tested_function(test_sentence)) == list)
    ...
    def test_outputs_only_sentences_containing_word_mouse_or_mice(self):
        ...
        self.assertEqual(expected_output, self.tested_function(test_sentences), 'ERROR: expected = {}, Actual = {}'.
                         format(expected_output, self.tested_function(test_sentences)))
